...
cmd:option('-gpuid',	    0,		        'GPU id for use')

cmd:option('-dataDir',	    '/home/wanglingfeng/tempory/qlw/NTIRE2017/code/data/',     'data directory')
cmd:option('-dataset',      'B100',        'test dataset')
cmd:option('-type',         'test', 	    'demo type: bench | test | val')
-- cmd:option('-valFrom',      791,            'validate from')
-- cmd:option('-valTo',        800,            'validate to')
cmd:option('-valFrom',      801,            'validate from')
cmd:option('-valTo',        900,            'validate to')

cmd:option('-save',         '.',            'Save as')

cmd:option('-mulImg',       255,            'multiply constant to input image')

cmd:option('-model',        'baseline_x2',   'substring of model name:baseline_x2')
cmd:option('-swap',         -1,             'Model swap')
cmd:option('-ensembleW',    '-1',           'Ensemble weight')
cmd:option('-scale',        3,              'scale factor: 2 | 3 | 4')
...
cmd:option('-selfEnsemble', 'true',        'enables self ensemble with flip and rotation')
...
if opt.type == 'bench' then
	...
	dataDir = paths.concat(opt.dataDir, 'dataset/Set5/Set5_test_LR', Xs)
